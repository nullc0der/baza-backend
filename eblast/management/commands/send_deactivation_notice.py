...
class Command(BaseCommand):
    help = "This command will send deactivation"
    " notice eblast to inactive users"
    ...
    def get_username_and_emails(self, send_test: bool = False) -> None:
        ...
        users = User.objects.filter(
            last_login__lte=now() - timedelta(days=90)).exclude(
                username='paulhyatt')
        username_and_emails = []
        ...
        for user in users:
            if hasattr(user, 'bazasignup'):
                ...
                if bazasignup.status == 'pending' \
                        or bazasignup.status == 'incomplete':
    ...
    def handle(self, *args: Any, **options: Any) -> Optional[str]:
        ...
        with open('deactivation_notice.csv', 'w') as csvfile:
