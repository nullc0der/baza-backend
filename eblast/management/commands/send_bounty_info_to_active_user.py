...
class Command(BaseCommand):
    help = "This command will send bounty info eblast to active users"
    ...
    def send_email(self, email_id: str, username: str) -> None:
        template = loader.get_template(
            'eblast/bounty_info_to_active_user.html')
        msg = EmailMultiAlternatives(
            'New bounty program from baza foundation',
            'Check html message',
    ...
    def get_username_and_emails(self, send_test: bool = False) -> None:
        ...
        users = User.objects.filter(
            last_login__gte=now() - timedelta(days=90)).exclude(
                username='paulhyatt')
        username_and_emails = []
        ...
        for user in users:
            status_code, data = authhelperclient.get_user_emails(user.username)
            if status_code == 200:
                for i in data:
                    if i['primary'] and\
    ...
    def handle(self, *args: Any, **options: Any) -> Optional[str]:
        ...
        with open('bounty_info.csv', 'w') as csvfile:
